...
public class MazeView extends DefaultViewer {
	...
	public MazeView(MazeController c, RobotDriver driver) {
		super() ;
		controller = c ;
		this.driver = driver;
	}
	...
	@Override
	public void redraw(MazePanel mp, StateGUI state, int px, int py, int view_dx,
			int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
		//dbg("redraw") ;
		switch (state) {
		case STATE_TITLE:
//			redrawTitle(mp.getBufferGraphics());
			break;
		case STATE_GENERATING:
//			redrawGenerating(mp.getBufferGraphics());
			break;
		case STATE_PLAY:
			// skip this one
			break;
		case STATE_FINISH:
//			redrawFinish(mp.getBufferGraphics());
			break;
		case STATE_LOSS:
//			redrawLoss(mp.getBufferGraphics());
		}
	}
...
//		centerString(gc, fm, "Total energy consumed is : " + driver.getEnergyConsumption(), 180);
//		centerString(gc, fm, "Path length is: " + driver.getPathLength(), 200);
//
//	}
///**
// * if the robot runs out of battery, this screen will pop up and inform the user that they have run out of battery.
// * @param gc
// */
//	void redrawLost(Graphics gc) {
//		// produce blue background
//		gc.setColor(Color.blue);
//		gc.fillRect(0, 0, Constants.VIEW_WIDTH, Constants.VIEW_HEIGHT);
//		// write the title
...
//		gc.setFont(smallBannerFont);
//		fm = gc.getFontMetrics();
//		centerString(gc, fm, "Awwwwwww, You Lost.", 160);
//		// write the instructions
//		gc.setColor(Color.white);
//		centerString(gc, fm, "Hit any key to restart", 300);
//
//
//		driver = controller.getDriver();
//		centerString(gc, fm, "Energy consumed is : " + driver.getEnergyConsumption(), 180);
//		centerString(gc, fm, "Path length is: " + driver.getPathLength(), 200);
//
//	}

	/**
	 * Helper method for redraw to draw lost screen, screen is hard coded
	 * @param gc graphics is the off screen image
	 */
//	void redrawLoss(Graphics gc) {
//		// produce blue background
//		gc.setColor(Color.blue);
//		gc.fillRect(0, 0, Constants.VIEW_WIDTH, Constants.VIEW_HEIGHT);
//		// write the title
...
//		FontMetrics fm = gc.getFontMetrics();
//		gc.setColor(Color.yellow);
//		centerString(gc, fm, "Uh oh!", 100);
//		// write some extra blufb
//		gc.setColor(Color.orange);
//		gc.setFont(smallBannerFont);
//		fm = gc.getFontMetrics();
//		centerString(gc, fm, "Oh no, robot ran out of gas. Tears.", 160);
//		// write the instructions
//		gc.setColor(Color.white);
//		centerString(gc, fm, "Hit any key to restart", 300);
//
