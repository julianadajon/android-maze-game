...
//			}
	...
	@Override
	public void redraw(MazePanel mp, StateGUI state, int px, int py,
...
//			return ;
	 ...
	 * @param mp MazePanel, encapsulates graphics handler to manipulate screen
	 */
	public void draw_map(MazePanel mp, int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze, boolean showSolution) {
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				if ((seencells.hasWall(x,y, CardinalDirection.West) || showMaze) && theCondition)
					...
					mp.drawLine(nx1, ny1, nx1, ny2);
			}
		
		if (showSolution) {
			draw_solution(mp, offx, offy, px, py) ;
		}
	}
	 ...
	 * @param gc to draw on
	 */
	public void draw_currentlocation(MazePanel mp, int view_dx, int view_dy) {
		mp.setColor("red");
...
//		mp.fillOval(ctrx-cirsiz/2, ctry-cirsiz/2, cirsiz, cirsiz);
		...
		mp.drawLine( ctrx, ctry, aptx, apty);
		mp.drawLine( aptx, apty, aptx2 + ptofx, apty2 + ptofy);
		mp.drawLine( aptx, apty, aptx2 - ptofx, apty2 - ptofy);
	}
	 ...
	 * @param mp, encapsulates graphics to draw lines on
	 * @param offx
	 * @param offy
	 */
	public void draw_solution(MazePanel mp, int offx, int offy, int px, int py) {
