...
//			System.exit(0) ;
			// TODO: implement code that makes sure that if MazeBuilder.build is called for same the skill level twice, 
...
//		
	...
	public void run() {
		...
		try {
			...
			System.out.println("mazeBuilder height: " + mazeConfig.getHeight());
			System.out.println("mazeBuilder width: " + mazeConfig.getWidth());
			
			mazeConfig.setMazecells(cells);
			mazeConfig.setMazedists(dists);

			System.out.println("BSP node in MazeBuilder run() is null: " + (root == null));
...
//			mazeConfig.setStartingPosition(startx, starty);
	...
	*/
	...
	private void saveMaze(MazeConfiguration m) {
		if (m.getWidth() == 4) {
			MazeFileWriter fileWriter = new MazeFileWriter();
			fileWriter.store("skill0", 4, 4, 0, 60,
					m.getRootnode(),
					m.getMazecells(),
					m.getDistArray(),
					0, 0);
		...
		if (m.getWidth() == 12) {
			MazeFileWriter fileWriter = new MazeFileWriter();
			fileWriter.store("skill1", 12, 12, 2, 600,
					m.getRootnode(),
					m.getMazecells(),
					m.getDistArray(),
					0, 0);
		...
		if (m.getWidth() == 15) {
			MazeFileWriter fileWriter = new MazeFileWriter();
			fileWriter.store("skill2", 15, 15, 2, 900,
					m.getRootnode(),
					m.getMazecells(),
					m.getDistArray(),
					0, 0);
		...
		if (m.getWidth() == 20) {
			MazeFileWriter fileWriter = new MazeFileWriter();
			fileWriter.store("skill3", 20, 15, 3, 1200,
					m.getRootnode(),
					m.getMazecells(),
					m.getDistArray(),
					0, 0);
